...
class ReservationNightController extends Controller
{
    ...
    {
        // FIX: передаём пустую модель — _fields использует $reservationNight->field ?? ''
        $reservationNight = new ReservationNight();
        return view('reservation_nights.create', compact('reservationNight'));
    }
    ...
    public function update(UpdateReservationNightRequest $request, $id)
    {
        $reservationNight = ReservationNight::findOrFail($id);
