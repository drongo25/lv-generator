...
class RegularPriceController extends Controller
{
    ...
    {
        // FIX: передаём пустую модель — _fields использует $regularPrice->field ?? ''
        $regularPrice = new RegularPrice();
        return view('regular_prices.create', compact('regularPrice'));
    }
    ...
    public function update(UpdateRegularPriceRequest $request, $id)
    {
        $regularPrice = RegularPrice::findOrFail($id);
